...
Um uns dieser Frage dennoch zu nähern, beginnen wir zunächst, die Hypothese $\mathsf{KvL}$ auf weitere Weisen zu charakterisieren. Es ist intuitiv klar, dass für eine feste Sprache $L$ die NP-Relationen für $L$, die optimalen Beweissysteme für $L$, und die NPTMs welche $L$ entscheiden, alle im breitesten „Zertifikatsschemata“ sind, und alle diese Berechnungsmodelle untereinander umgeschrieben werden können.

Wir wollen nun die existierende $\leqlp$-Ordnung auf den NP-Relationen nehmen, und diese auf die optimalen Beweissysteme übertragen. Wir definieren hierfür eine abgeschwächte Variante der $\P$-Simulation, welche der $\leqlp$-Reduktion nachempfunden ist.
\begin{definition}
...
Die intuitive Aussage von $\neg\mathsf{KvL}$ „Alle NP-Relationen für NP-vollständiges $L$ sind im Wesentlichen gleich“ überträgt sich nun auch auf optimale Beweissysteme:
\begin{theorem}
    Folgende Aussagen sind äquivalent:
    \begin{enumerate}
        \item Aussage $\neg\mathsf{KvL}$.
        \item Für alle NP-Relationen $R$ mit $\Proj(R)$ $\leqmp$-vollständig, alle NPTM $N$ mit $L(N)=\Proj(R)$ gilt
            \[ R \leqlp \{ (x, w) \mid  \text{$N(x)$ akz. auf Rechenweg $w$}\}. \]
        \item Für alle NP-Relationen $R$ mit $\Proj(R)$ $\leqmp$-vollständig, alle optimalen Beweissysteme $h$ mit $\img(h)=\Proj(R)$ gilt
            \[ R \leqlp \{ (x, w) \mid  \text{$h(w)=x$, $|w|\leq q(|x|)$}\}, \]
            wobei $q$ ein geeignetes Polynom ist.
        \item Für alle NP-Relationen $R$ mit $\Proj(R)$ $\leqmp$-vollständig, alle optimalen Beweissysteme $h$ mit $\img(h)=\Proj(R)$ gilt
    \[  \text{$\mathit{std}_R$ $\P$-simuliert effektiv $h$}. \]
        \item Für alle NP-Relationen $R$ mit $\Proj(R)$ $\leqmp$-vollständig ist $\mathit{std}_R$ effektiv $\P$-optimal. 
    \end{enumerate}
\end{theorem}
\begin{proof}
\todo{}
...
Wir haben damit also je eine notwendige ($\neg\hQ$) und eine hinreichende Hypothese ($\mathsf{SAT^{eff}}$) für $\mathsf{KvL}$. Durch die Charakterisierung von $\neg\mathsf{KvL}$ durch Satz~\ref{} über Beweissysteme ergibt sich eine interessante Ähnlichkeit zur Hypothese $\hQ$. Wir wollen diese beiden Hypothesen im Folgenden kurz gegenüberstellen.
Zur Anschaulichkeit beziehe ich mich hierbei auf die NP-Relation $\mathtt{rSAT}$. (Durch Ergebnisse im nächsten Abschnitt wird klar, dass sich dieses Argument auch auf $\mathtt{rKAN}$ überträgt.)

Wir haben
\begin{itemize}[nosep]
    \item $\hQ$ genau dann wenn $\mathit{sat}$ $\P$-optimal ist,
    \item $\neg\mathsf{KvL}$ genau dann wenn $\mathit{sat}$ effektiv $\P$-optimal ist.
\end{itemize}
Insbesonere vermute ich, dass möglicherweise sogar $\hQ\Leftrightarrow\neg\mathsf{KvL}$ gelten könnte. Die Richtung von links nach rechts haben wir bereits in Satz~\ref{} gesehen. Für die Richtung von rechts nach links möchte ich nun plausibilisieren, dass $\neg\mathsf{KvL}\land\neg\hQ$ zu einem Widerspruch führt.

Unter Annahme von $\neg\hQ$ folgt nach Satz~\ref{}, dass das Standardbeweissystem $\mathit{sat}$ von $\mathtt{rSAT}$ nicht $\P$-optimal ist.
Insbesondere existiert ein optimales Beweissystem $h$, sodass $\mathit{sat}$ nicht $h$ $\P$-simulieren kann.
...
Nun gilt aber nach Annahme auch $\neg\mathsf{KvL}$. Über Satz~\ref{}(?) gilt, dass $\mathit{sat}$ das optimale Beweissystem $h$ effektiv $\P$-simulieren kann. Wir haben also zwei Funktionen $f', g'\in\FP$ sodass
...
Später werden wir sehen, welche NP-Relationen alle diese Eigenschaft der Levin-Paddability erfüllen. Festhalten können wir aber, dass $\mathtt{rSAT}$ Levin-paddable ist. Das ist einfach zu sehen: padde Formeln $\phi$ auf, indem z.B. Disjunktionen neue Variablen hinzugefügt werden, also z.B. 
...
Wir wollen nun eine möglichst breite Klasse an NP-Relationen angeben, für die diese beiden obigen Äquivalenzen gelten, also insbesondere diejenigen NP-Relationen, welche die selbe Charakterisierung wie $\mathtt{rSAT}$ im Fall der unrelativierbaren Charakterisierung von \citeauthor{fenner_inverting_2003} und \citeauthor{messner_simulation_2001} zulassen.
...
\begin{observation}\label{obs:invcomplete-sind-levinpaddable}
    ...
    \begin{enumerate}
        ...
        \item Jede $\leq_\mathrm{L,1,i}^\mathrm{p}$-vollständige NP-Relation $R$ ist auch Levin-paddable.
    \end{enumerate}
\end{observation}
Damit können wir schon als Ergebnis festhalten, dass 
jede $\leq_\mathrm{L,1,i}^\mathrm{p}$-vollständige Relation $R$ die in 
Lemma~\ref{lemma:stdps-q} und~\ref{lemma:q-generalized} genannten Voraussetzungen an die NP-Relation $R$ erfüllt.
Das sind nach \textcite{goldreich_computational_2008} im unrelativierten Fall u.a. $\mathtt{rSAT}$, $\mathtt{rSETCOVER}$, $\mathtt{rVC}$, $\mathtt{rCLIQUE}$, $\mathtt{r3COLORABILITY}$.

\medskip
\begin{proof}[Beweis zu Beobachtung~\ref{obs:invcomplete-sind-levinpaddable}]
    Aussage (2) folgt unmittelbar aus (1): Wir haben $\mathtt{rKAN}\leq_\mathrm{L,1,i}^\mathrm{p} R$ und damit ist die entsprechende Reduktionsfunktion $f$ $\P$-invertierbar, und damit ehrlich.
...
Im Kontext der Frage, welche natürlichen NP-Relationen Levin-vollständig und Levin-paddable sind, sei auch noch auf den Begriff der \emph{Universalität} hingewiesen \textcite{agrawal_universal_1992}, welcher schon kurz in Abschnitt~\ref{sec:levin} angesprochen wurde. Es ist leicht nachzuweisen, dass uiverselle Relationen sowohl Levin-vollständig und Levin-paddable sind.
Das sind nach \textcite{agrawal_universal_1992} im unrelativierten Fall u.a. $\mathtt{rSAT}, \mathtt{rHAMCYCLE}, \mathtt{rINDSET}, \mathtt{rKNAPSACK}, \mathtt{rMAXCUT}$.


Es bleibt trotzdem die Frage offen, ob Levin-Paddability für \emph{alle} vollständigen NP-Relationen zutrifft. Unter Annahmen einer geeigneten Einwegfunktion ist dies nicht der Fall.
...
%\item (10)$\implies$(5): Sei $L\in \P$ und sei $N$ eine NPTM mit $L(N)=L$, wobei das Polynom $q$ die Laufzeit beschränkt. Wir wollen eine Funktion $h\in\FP$ definieren sodass $h(x)$ ein akzeptierender Rechenweg von $N(x)$ für $x\in L$ ist. Definiere die NP-Relation
    ...
    %Nachdem (10) gilt, haben wir eine $\leq_\mathrm{L,1,i}^\mathrm p$-vollständige NP-Relation $R$. Damit gilt $Q\leqlp R$ mittels Reduktions- bzw. Translationsfunktion $f, k\in FP$. Insbesondere existiert eine Inverse $f^{-1}\in\FP$ zu $f$.
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        %\item Für alle $\leq_\mathrm{L,1,i}^\mathrm p$-vollständigen NP-Relationen $R$, für alle Mengen $S\in \P$ mit $S\subseteq \Proj(R)$ gilt: es existiert eine Funktion $g\in\mathrm{FP}$ sodass
            %\[ x\in S \implies (x, g(x))\in R. \]
        \item[(1$\,'$)] $\neg\hQ$.
        ...
        %\item[(4$'$)] Es existiert keine $\leq_\mathrm{L,1,i}^\mathrm p$-vollständige NP-Relation $R$, sodass für alle Mengen $S\in \P$ mit $S\subseteq \Proj(R)$ gilt: es existiert eine Funktion $g\in\mathrm{FP}$ sodass
            %\[ x\in S \implies (x, g(x))\in R. \]
    \end{enumerate}
...
%\item (8$'$)$\implies$(1), (9$'$)$\implies$(1), (10$'$)$\implies$(1):
    %Die NP-Relation $\mathtt{rKAN}$ ist $\leq_\mathrm{L,1,i}^\mathrm p$-vollständig (und damit auch Levin-paddable).
