...
\begin{proof}
    ...
    \begin{algorithm}[H]
        \lIf{$T$ kein Transduktor ist}{Gebe $\epsilon$ aus}
        ...
        \lIf{$T(x)$ auf $\alpha$ nicht terminiert, heißt ein weiterer Rechenschritt ist möglich}
        { Gebe $\epsilon$ aus }
        ...
        \lElseIf{$N_u(i, x, 1^n)$ mit $\beta$ akzeptiert}
        {
            Gebe $\epsilon$ aus
        }
        \lElse
...
Symmetrisch sehen wir über eine Variante eines Orakels von \textcite{dose_further_2020}, dass die Anname $\neg\hQ$ (bzw. sogar die stärkere Annahme $\neg\hQ'$) nicht aussreicht, um $\hTAUT$ oder $\hSAT$ zu zeigen, also auch nicht um die stärkeren ursprünglichen Pudlákschen Hypothesen ($\hDisjNP, \hNPcoNP, \hDisjCoNP$, usw.) zu beweisen.\marginnote{\todo{sollten wir irgendwo hinschreiben!}}
