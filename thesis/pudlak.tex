...
In anderen Worten sagt Satz~\ref{thm:q-fenner} aus, dass unter Annahme von $\hQ$ es modulo Umcodieren nur einen einzigen SAT-Solver gibt, und insbesondere alle SAT-Solver äquivalent zum trivialen Solver ist, welcher nur alle möglichen Belegungen ausprobiert.
...
\begin{proof}
    \begin{prooflist}
    \item (1)$\implies$(2): Klar, denn die Relation $R_N$ ist eine NP-Relation und es gilt $\Proj(R_N)=L(N)=\Proj(R)$ und damit die Projektion von $R$ $\leqmp$-vollständig. Nach Aussage (1) gilt also nun, dass $R\leqlp R_N$, wie gewünscht.
    \item (2)$\implies$(3): Sei $h$ ein solches erhliches Beweissystem, und $q$ so gewählt, dass $q(|f(w)|)\geq |w|$. Betrachte nun die NPTM $N$, welche auf Eingabe $x$ einen Beweis $w$ der Länge $\leq q(|x|)$ rät, und genau dann akzeptiert wenn $f(w)=x$.
        ...
        \intertext{wobei $\alpha_w$ derjenige Rechenweg sein soll, welcher den Beweis $w$, $|w|\leq q(|f(w)|)=q(|x|)$ rät,}
    ...
    \item (3)$\implies$(4): 
        Sei $h$ ein solches ehrliches Beweissystem, und $q$ so gewählt, dass $q(|f(w)|)\geq |w|$. Nach (3) gilt $R\leqlp R_{h}$.
        Seien $f, g\in\FP$ die Reduktions- bzw. Translationsfunktion, welche diese $\leqlp$-Reduktion realisieren.

        Nun gilt $x\in \Proj(R) \iff f(x)\in \Proj(R)$, und wir haben
    ...
    \item (4)$\implies$(1): Sei $R$ eine NP-Relation, wobei $\Proj(R)$ $\leqmp$-vollständig ist. Wir werden zeigen, dass $\mathtt{rKAN}\leqlp R$, wobei dann auch $R$ $\leqlp$-vollständig ist (Beob.~\ref{lemma:fnp-completeness}), wie gewünscht.
        ...
        \[ h(z,y) = \begin{cases} z & \text{falls $(f(z), y)\in R$}\\ \bot & \text{sonst}. \end{cases}\]
        ...
        Seien hierfür $z, y$ gegeben sodass $h(z,y)=z$. Damit gilt $(f(z), y)\in R$, also
        \[ |(z,y)| = 2+2|z|+2|y| \leq 2+2|z|+2q(|f(z)|) \leq 2+ 2|z|+ 2q(p(|z|)), \]
        und damit die Ausgabe $z$ nicht superpolynomiell kürzer als die Eingabe $(z,y)$.
        ...
        Wir haben
        \[ (f(f'(z)), y)\in R \implies h(f'(z),y)=f'(z) \implies \mathit{std}_\mathtt{rKAN}(g'(f'(z), y))=z, \]
        und wir können insbesondere können wir $g'(f'(z), y)$ als $(z, w)$ schreiben. Dann gilt $(z,w)\in \mathtt{rKAN}$.
        Die Rückübersetzung $g''(z,y)=(z,w)$ ist dabei effizient möglich.
...
\begin{conjecture}[$\mathsf{SAT^{eff}}$]
    Keine $\leqmp$-vollständige Menge $L$ für $\NP$ hat ein Beweissystem $h$, welches alle anderen ehrlichen Beweissysteme für $L$ effektiv $\P$-simulieren kann. 
\end{conjecture}
...
\begin{proof}
\begin{prooflist}
\item Zu (1): Klar aus Kontraposition. Wenn $\neg\hSAT$, dann existiert für eine $\leqmp$-vollständige Menge $L\in\NP$ ein $\P$-optimales Beweissystem $h$ für $L$ existiert, dann kann dieses $h$ auch alle anderen Beweissysteme $\P$-simulieren, und damit insbesondere auch alle ehrlichen Beweissysteme $h'$ effektiv $\P$-simulieren.
...
Wir haben
\begin{itemize}[nosep]
    \item $\hQ$ genau dann wenn $\mathit{sat}$ $\P$-optimal ist (Satz~\ref{thm:q-messner}),
...
Unter Annahme von $\neg\hQ$ folgt nach Satz~\ref{thm:q-messner}, dass das Standardbeweissystem $\mathit{sat}$ von $\mathtt{rSAT}$ nicht $\P$-optimal ist.
Ohne Beschränkung existiert dann ein ehrliches Beweissystem $h$, sodass $\mathit{sat}$ nicht $h$ $\P$-simulieren kann.\sidenote{Die Ehrlichkeit dieses Beweissystems kann im Fall von $\mathtt{rSAT}$ über ein einfaches Padding-Argument gezeigt werden. Alternativ induziert die NPTM $N$ aus Satz~\ref{thm:q-fenner} ein ehrliches Beweissystem, das nicht von $\mathit{sat}$ $\P$-simuliert werden kann.}
...
\begin{proof}
    \begin{prooflist}
        \item (1)$\implies$(2): Klar, denn mit Satz~\ref{thm:q-messner-generalized} folgt aus $\hQ$, dass $\mathit{std}_R$ $\P$-optimal ist, heißt es gilt auch $\mathit{std}_N\leqmp \mathit{std}_R$.

        \item (2)$\implies$(1): Sei zunächst $p$ die Zertifikatsschranke von $R$. Wir zeigen erneut $\TFNP\subseteqc\FP$ für $\hQ$.
...
Für die üblichen natürlichen NP-Relationen (wie $\mathtt{rSETCOVER}$, $\mathtt{rVC}$, $\mathtt{rCLIQUE}$, $\mathtt{r3COLORABILITY}$, $\mathtt{rHAMCYCLE}$, $\mathtt{rMAXCUT}$) ist leicht zu sehen, dass diese alle Levin-paddable sind. 
