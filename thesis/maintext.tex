...
\todo{Maschinenenumerierungen definieren!}

\section{Zum Verfahren der Orakelkonstruktion}

Um die beiden Orakel zu konstruieren, wenden wir ein Verfahren von \textcite{dose_np-completeness_2019} an, welche sich als ein starkes Framework erwiesen hat, Orakel zu konstruieren die simultan mehrere Eigenschaften erfüllen.
Bevor wie die zwei eigentlichen Konstruktionen angeben, sei hier kurz skizziert wie dieses Verfahren abläuft.

Jede der beiden Konstruktionen läuft in $\omega$-geordneten \emph{Stufen} ab, wobei sukzessiv die Zugehörigkeit von natürlichen Zahlen $x\in \mathbb N$ zum finalen Orakel $O$ fixiert wird.
Im Laufe der Konstruktion haben wir es also mit Orakeln zu tun, welche „partiell definiert“ sind, d.h. zu denen $x\in O$ bwz. $x\not\in O$ für gewisse $x$ noch nicht festgelegt ist.
...
Die intendierte Interpretation ist, dass gegenüber der \emph{Menge} $w$ die Zugehörigkeit von Zahlen (bzw. äquivalent Wörtern) $x$ mit $x<|w|$ final speizifiziert ist, nicht dagegen für $x\geq|w|$ (und nur ersatzweise $x\not\in w$ gilt).


Diese Interpretation von finiten Wörtern $w$ als Orakel macht es einfacher, unsere Orakelkonstruktionen präzise und knapp zu beschreiben. In den meisten der oben genannte Stufen werden wir $w$ so erweitern, dass die Zugehörigkeit des kleinsten $x\in\mathbb N$ spezifiziert wird, welche noch nicht final spezifiziert ist. Dieses $x$ ist genau $|w|$ und wir legen die Zugehörigkeit final fest, indem wir an $w$ entweder $0$ anhängen (und $x$ ist final nicht im Orakel $w0$) oder $1$ anhängen (und $x$ ist final im Orakel $w1$).
...
\begin{observation}\phantomsection\label{obs:partialoracles}
    \begin{enumerate}
        \item Wenn $M^w(x)$ eine definite Berechnung ist und $v\sqsupseteq w$ ist, dann ist $M^v(x)$ definite. Die Berechnung $M^v(x)$ akzeptiert genau dann, wenn $M^w(x)$ akzeptiert.
        \item Wenn $w$ für alle Wörter der Länge $p_i(|x|)$ definiert ist, dann ist $M_i^w(x)$ definite.
...
Nun zurück zur Konstruktion. 
Allgemein legen wir in unserer Konstruktion eine induktive Definition fest. Wir starten mit einem initialen partiell definierten Orakel $w_0 = \epsilon$ (welches die Zugehörigkeit von keinem Wort festlegt) als Basisfall, dieses werden wir nach einer Definitionsvorschrift zu $w_1\sqsupsetneq w_0$ erweitern (damit also die Zugehörigkeit von mehr Wörtern final festlegen), dann zu $w_2\sqsupsetneq w_1$ erweitern usw.
Wir erhalten also iterativ eine Sequenz
\[ \{w_i\}_{i<\omega}, \quad w_0\sqsubsetneq w_1 \sqsubsetneq w_2 \sqsubsetneq w_3 \sqsubsetneq \cdots \]
von partiell definierten Orakeln.
Setze nun
\[ O = \bigcup_{i<\omega} w_i \]
wobei hier also $w_i$ je als (endliche) Menge interpretiert wird.
Dieses Orakel $O$ ist dann unser abschließendes Orakel.

Damit das Orakel $O$ auch die gewünschten Eigenschaften erfüllt, fügen wir noch zwei weitere Typen von Constraints in die Konstruktion ein.
Die ersten sind eine abzählbar unendlich große Liste von \emph{Tasks}, $\tau_1, \tau_2, \ldots$ die abgearbeitet werden müssen. Das tritt zum Beispiel bei sogenannten Diagonalisierungen auf, bei der also z.B. für jede PTM $M_1, M_2, \ldots$ gesichert werden muss, dass diese relativ zu $O$ eine Zeugenmenge $L\in\NP^O$ nicht entscheiden -- dann wäre $\P^O\neq\NP^O$.
Jede dieser Tasks $\tau_i$ muss dann durch eine Stufe $w_{i'}$ abgearbeitet werden. Natürlich muss dabei gesichert werden, dass dieser Task durch Verhalten in den höheren Stufen nicht unwirksam gemacht wird. Unser Begriff der „definiten Berechnungen“ wird hierbei dienlich sein.

Die zweiten Constraints sind \emph{Validitätsbedingungen}. 
Hierbei setzen wir voraus, dass jedes $w_i$ gewisse Eigenschaften sicherstellen muss.
In unserer induktiven Definition wird die Anzahl der zu sichernden Eigenschaften sogar in jedem Schritt anwachsen.
In bisherigen Konstruktionen hat es sich als dienlich erwiesen, diese Bündel an Eigenschaften je über eine (partielle) Funktion 
