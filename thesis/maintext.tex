...
\begin{proof}[Beweis zu Beobachtung~\ref{obs:joinable-sind-levinpaddable}]
    ...
    An dieser Stelle könnten wir (mit Definition ??) über die Länge $|\delta|$ die Länge von Zertifikaten zu $x'$ schließen, und damit $|x'|$ beschränken.
...
\begin{theorem}[Äquivalente Formulierungen der Hypothese $Q$]
    Folgende Aussagen sind äquivalent:
    \begin{enumerate}
...
\item (10)$\implies$(5): Sei $L\in \P$ und sei $N$ eine NPTM mit $L(N)=L$, wobei das Polynom $q$ die Laufzeit beschränkt. Wir wollen eine Funktion $h\in\FP$ definieren sodass $h(x)$ ein akzeptierender Rechenweg von $N(x)$ für $x\in L$ ist. Definiere die NP-Relation
    \[ Q = \{ ((N, x), y) \mid \text{$N(x)$ akzeptiert mit Rechenweg $y\in\Sigma^{\leq q(|x|)}$} \}. \]
    ...
    \begin{gather*}x\in L \implies f(N, x) \in S \implies (f(N,x), g(f(N, x)))\in R\\ \implies ((N, x), k(g(f(N, x))))\in Q\\
    \implies N(x) \text{ akz. mit Rechenweg $\underbrace{k(g(f(N, x)))}_{h(x)}$. }
\end{gather*}
    Definiere nun die gesuchte Funktion $h\in \FP$ mit $h(x) = k(g(f(N, x)))$. Damit gilt für alle $x\in L$ dass $N(x)$ mit Rechenweg $h(x)$ akzeptiert, wie gewünscht.
