...
\[ \mathtt{KAN}^O \defeq \{ (N, x, 1^n) \mid \text{$N$ ist eine NOTM und akz. $x$ relativ zu $O$ auf einem RW mit $\leq n$ vielen Schritten} \}. \]
...
\begin{quote}
    ...
    (Dieser existiert immer, denn es existieren nur endlich viele Knoten.)
\end{quote}
...
\begin{proof}
    ...
    i.e, $q(x)$ ist ein Zertifikat für $x$, wie gewünscht. Hierbei folgt die dritte Implikation nach Definition~\ref{def:levin-reduction}(2).
\end{proof}
...
\begin{proof}[Skizze.]
    ...
    Wir haben nun
    \[ \phi\not\in\mathtt{SAT} \iff |\fset{}\mathtt{rSAT}(\phi)|=0 \iff |\fset{}\mathtt{rSAT}(\phi')|=1 \iff |\fset{}\mathtt{r4CHROMINDEX}(f(\phi'))|=1 \iff f(\phi') \in A, \]
