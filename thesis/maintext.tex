...
\begin{definition}[NP-Relation]
    ...
    Wir schreiben $\mathrm{FNP}$ für die Klasse aller NP-Relationen.
\end{definition}

%Interpreting each NP relation as function, we see that the class $\mathrm{FNP}$ coincides with the “textbook definition” of $\mathrm{FNP}$, e.g., as defined by \textcite[cf.][227-228]{papadimitriou_computational_1994}. This makes $\mathrm{FNP}$ the non-total variant of $\TFNP$. 
%Discussion on the complexity of function computation goes back at least to \textcite{valiant_relative_1976}, who first observed the equivalence of the P–NP question with a respective variant of function problem classes, in particular
%\[ \NP\subseteq\P \iff \mathrm{FNP}\subseteq_c \mathrm{FP}. \]
%It needs to be remarkted that the notation of function classes, search problem clases, etc. is not consistent throughout the literature. \textcite{valiant_relative_1976}, in its original formulation, denoted the class of NP relations as $\mathrm{PC^p}$ (“polynomially checkable with polynomially long function output“). This notation has been refined by \textcite{selman_taxonomy_1994} to $\mathrm{NPMV_g}$ in its taxonomy on function clases, highlighting that NP relations are “partial multivalued functions computed by NPTM transducers” with the subscript $g$ marking that the relation is in $\P$. In this taxonomy, $\mathrm{NPMV_g}$ can be contrasted to $\mathrm{NPSV_g}$ (where NP relations are single-valued, i.e., right-unique) and $\mathrm{NPMV}$, $\mathrm{NPSV}$ (where P-checkability is dropped).
%As already indicated above, \textcite{megiddo_total_1991} define the class $\mathrm{FNP}$ precisely as given here, in particular to contrast this class to the subset $\mathrm{TFNP}$, defined as the set of (left-)total NP relations, that is, where every $x\in\Sigma^*$ is a problem that has a witness. This is the usual interpretation of $\mathrm{TFNP}$ in the literature. The notation proposed \textcite{wechsung_vorlesungen_2000} would denote the class of NP relations as $\mathrm{Rel}\,\P$ highlighting that NP relations are \emph{relations}, rejecting the notion of “multivalued functions” and reserving the label $\mathrm{Fun}\,\P$ only for the class of NP relations that are right-unique, i.e., relations where every problem $x$ has at most one witness $y$; in other words, \emph{partial functions}. This is identical to the class $\mathrm{NPSV_g}$ as defined by \textcite{selman_taxonomy_1994}.
