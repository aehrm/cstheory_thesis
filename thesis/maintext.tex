...
\begin{proof}
    ...
    \begin{algorithm}[H]
        ...
        \uIf{$(x, R)\not\in A$}{\Return{ablehnen}}
        \tcc{Es existiert also ein kleinster Zeuge $y$ zu $x$.}
        \While(\tcp*[h]{Invariante: $L\leq y\leq R$}){$L<R$}
        {
            $M\gets \lfloor (L+R)/2 \rfloor$\;
            \uIf(\tcp*[h]{damit $L\leq y \leq M \leq R$}){$(x, M)\in A$}{
                $R\gets M$\;
            }
            \Else(\tcp*[h]{damit $L\leq M<y \leq R$}){
                $L\gets M+1$\;
            }
        }
        \Return{$L$}
