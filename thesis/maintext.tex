...
\begin{proof}
    ...
    \[ A = \{ (x,y) \mid \exists w\in\Sigma^{\leq q(|x|)-|z|}, (x, yw)\in R \} \]
    ...
    \begin{algorithm}[H]
        $y\gets\epsilon$\;
        \While(\tcp*[h]{Invariante: wenn ein Zertifikat $z$ für $x$ ex., dann $y\sqsubseteq z$}){$|y|\leq q(|x|)$}
        {
            \uIf{$(x,y)\in R$}{\AcceptWith{$y$}}
            \uElseIf{$(x,y0)\in A$}{$y\gets y0$}
            \uElseIf{$(x,y1)\in A$}{$y\gets y1$}
            \Else{\Reject}
        }
        \Reject
    \end{algorithm}
...
%\begin{lemma}
    %$\mathrm{TFNP}\subseteqc \FP \iff \mathsf{Q}$
%\end{lemma}

\begin{corollary}[\emph{Search reduces to decision} für die NP-Vollständigen]
...
\todo{Suchprobleme können natürlich immer auf die Bisektions-Entscheidunsvariante reduziert werden, z.B. rSMALLFACTOR. Das sind dann aber nicht immer natürliche NP-Relationen mehr. Andererseits ist es z.B. bei Graphisomorphismus möglich, Suche auf das (natürliche) Entscheidungsproblem zu reduzieren}

%\begin{theorem}
    %Es existiert eine totale NP-Relation $R$ mit $R\not\inc \FP^{\Proj(R)}$ genau dann wenn eine totale ehrliche Funktion $g\in\FP$ existiert die nicht p-invertierbar ist. (Vgl. Hypothese $\mathsf{Q}$.)
%\end{theorem}
Abschließend wollen wir noch theoretische Resultate präsentieren. Die ersten zwei plausbilisieren, dass wahrscheinlich eine NP-Relation existiert, für die das Suchproblem nicht auf das Entscheidungsproblem reduziert werden kann. 
...
Für (totale) Suchprobleme aus $\TFNP$ kann nicht sinnvoll gefragt werden, ob hier das Suchproblem auf das Entscheidungsproblem reduziert werden kann, ist ja für $R\in\TFNP$ das entsprechende Entscheidungsproblem $\Proj(R)=\Sigma^*$ trivial.
...
Betrachte hierbei folgende Variante eine POTM-Transduktors relativ zu $R$: Dieser Transduktor ist wie ein üblicher PTM-Transduktor, hat zusätzlich aber Zugriff auf ein \emph{funktionales} Orakel, in dem Sinne dass er Orakelfragen der Form „\emph{gib mir ein Zertifikat $y$ für $x'$}“ stellen kann. Das Orakel antwortet dann
\begin{itemize}
    \item mit einem solchen Zerifikat $y$ falls ein solches mit $(x', y)\in R$ existiert, oder
    \item mit „unlösbar“ falls ein solches Zertifikat nicht existieren sollte.
\end{itemize}
...
\todo{Beispiel eines Problems (muss in PLS liegen) das nach unten selbstreduzierbar ist}

Für die TFNP-Relation $\mathtt{rFACTOR}'\in\TFNP$ (i.e., gebe mir einen nichttriviane Faktor für $n$, oder gebe „prim“ aus) zeigen \citeauthor{harsha_downward_2023}, dass diese Relation wahrscheinlich nicht nach unten selbstreduzierbar ist.
