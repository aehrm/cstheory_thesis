...
Wir werden später zeigen, dass natürliche NP-Relationen die mutmaßlich nicht $\leq_\mathrm{pars}^\mathrm p$-vollständig sind, und damit auch nicht zertifikats-isomorph zu $\mathtt{rSAT}$ sein können.
...
Die Eigenschaft \emph{universell} stellt eine Verstärkung der Levin-Vollständigkeit da, und ist eine der stärksten strukturellen Eigenschaften, die (zumindest von \citeauthor{agrawal_universal_1992}) bei allen natürlichen Levin-vollständigen NP-Relationen (modulo einer trivialen Umcodierung der Zertifikate) vermutet wird. Daher wird deren Arbeit hier auch extensiver ausgeführt.
...
Zertifikate für $\phi$ sind dann Strings $w$ der Form $\Sigma^{|\phi|}$ sodass $\phi$ durch die Variablenbelegung $x_0=w[0], x_1=w[1], \dots$ erfüllt wird. Beachte, dass wenn $x_j$ nicht in $\phi$ vorkommt, dass dann $(\phi, w)\in\mathtt{rSAT})$ genau dann wenn $(\phi, w')\in\mathtt{rSAT}$, wobei $w$ und $w'$ sich nur in der $j$-ten Stelle unterscheidet.
...
\begin{definition}[Projektive Levin-Reduktion]
    ...
    \end{enumerate}
\end{definition}
Es ist leicht zu sehen, dass aus einer projektiven Levin-Reduktion von $Q$ auf $R$ auch $Q\leqlp R$ folgt. 
...
\begin{theorem}[\cite{agrawal_universal_1992}]
...
\end{theorem}
\textcite{agrawal_universal_1992} haben explizit verifiziert, dass $\mathtt{rSAT}$, $\mathtt{rHAMCYCLE}$, $\mathtt{rINDSET}$, $\mathtt{rKNAPSACK}$, $\mathtt{rMAXCUT}$ alle universell sind.
...
Konkret wurde aber nicht exhaustiv untersucht, welche der natürlichen NP-Relationen universell sind und welche nicht. Die oben genannte Argumentation wollen wir aber im Folgenden zumindest so deuten, dass selbst die Existenz einer natürlichen $\leqlp$-vollständigen NP-Relation, die nicht universell ist, sehr unwahrscheinlich ist.

Im restlichen Teil dieses Abschnittes werden wir die unterschiedlichen Vollständigkeitsbegriffe nach ihrer Stärke ordnen, und die Universalität von $\leq_\mathrm{pars}^\mathrm p$-Vollständigkeit trennen.

\begin{figure}
    \centering\includegraphics[page=2]{figures.pdf}
    \caption{Implikationen zwischen den  Vollständigkeitsbegriffen. Ein unterbrochener Pfeile von $\mathsf{A}$ nach $\mathsf{B}$ sagt aus, dass ein Gegenbeispiel für die Implikation $\mathsf{A\Rightarrow B}$ existiert, also für das $\mathsf{A}$ und $\neg\mathsf{B}$ gilt.}
\end{figure}
