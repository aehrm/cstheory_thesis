...
\begin{figure}
    ...
    \caption{\todo{TODO}}\label{fig:3col}
\end{figure}
...
%\begin{itemize}
    %\item Will $\hQ$ in den Pudlák-Baum einordnen: dafür ist es notwendig, diese ordentlich zu relativieren. Insb. will ich zeigen, dass einige bisherige Resultate natürlicherweise auf „Standardbeweissysteme“ vollständiger Mengen übertragen (nicht nur das Standardbeweissystem für SAT).
%\end{itemize}

\todo{Intro}

Da wir im späteren Teil der Arbeit mit Orakeln die einzelnen Hypothesen (unter relativierbaren Beweisen) trennen wollen, ist es notwendig, dass wir uns bei den einzelnen Hypothesen auf eine relativierbare Definition einigen.\marginnote{\todo{Wo sollen wir eigentlich UP, DisjNP, DisjCoNP definieren?}}

Unter den Hypothesen betreffen der Existienz von vollständigen Sprachen bzw. Paaren ($\hUP$, $\hDisjNP$, \dots) ist eine natürliche Relativierung klar. Beispielsweise wäre die zu Orakel $O$ relativierte Hypothese $\hUP$ die Aussage, dass keine $\leq_\mathrm{m}^{\mathrm p,O}$-vollständige Menge für $\UP^O$ existiert.

Für die drei Hypothesen $\hSAT$, $\hTAUT$ und $\mathsf{CON^N}$ ist das nicht klar. Zur Erinnerung:
wir definierten in der Einleitung, \textcite{pudlak_incompleteness_2017} folgend,
\begin{align*}
    \hSAT : \quad&\text{es existiert kein p-optimales Beweissystem für $\mathtt{SAT}$},\\
    \hTAUT : \quad&\text{es existiert kein p-optimales Beweissystem für $\mathtt{TAUT}$},\\
    \mathsf{CON^N} : \quad&\text{es existiert kein optimales Beweissystem für $\mathtt{TAUT}$}.
\end{align*}
Gleichzeitig haben wir uns dazu entschieden, keine relativierte Variante der Sprachen $\mathtt{SAT}$ bzw. $\mathtt{TAUT}$ zu definieren.
Anstelle von einer konkreten $\leqmp$-vollständigen Menge $\mathtt{SAT}$ zu sprechen, können wir stattdessen die Aussage auf alle Mengen in $\NP$ ausweiten, um so nicht mehr von einer konkreten Menge zu sprechen. Insbesondere haben wir folgende (relativierbare!) Beobachtung:
\begin{observation}
    Folgende Aussagen sind äquivalent:
    \begin{enumerate}
        \item Es existiert eine $\leqmp$-vollständige Menge $A$ für $\NP$, für welche ein (p-)optimales Beweissystem $h$ existiert.
        \item Für jede Menge $B\in\NP$ existiert eine (p-)optimales Beweissystem.
    \end{enumerate}
\end{observation}
\begin{proof}
    Die Richtung (2)$\Rightarrow$(1) ist klar. Für die Richtung (1)$\Rightarrow$(2) reicht die Aussage von Lemma~\ref{lemma:optimal-downward} dass sich die Existenz von (p-)optimalen Beweissystemen auf $B\leqlp A$ nach unten überträgt. Da $A$ vollständig ist, also für alle $B\in\NP$.
\end{proof}
Mit dieser äquivalenten Charakterisierung bietet es sich an, mit folgenden relativierten Varianten der oben genannten Hypothesen arbeiten, welche als erstes von \textcite{dose_oracle_2020} vorgeschlagen wurde: relativ zu $O$ sei
\begin{align*}
    \hSAT : \quad&\text{es existiert kein $L\in\NP^O$ mit einem p-optimales Beweissystem $f\in\FP^O$ für $L$},\\
    \hTAUT : \quad&\text{es existiert kein $L\in\coNP^O$ mit einem p-optimales Beweissystem $f\in\FP^O$ für $L$},\\
    \mathsf{CON^N} : \quad&\text{es existiert kein $L\in\coNP^O$ mit einem optimales Beweissystem $f$ für $L$}.
\end{align*}
Im Folgenden werden wir nur noch mit diesen relativierbaren Hypothesen arbeiten. Damit ist auch klar, wenn wir von relativierbaren Implikationen $\mathsf{X}\Rightarrow \hSAT$ oder Orakel-Trennungen $\mathsf{Y}\land\neg\hSAT$ sprechen.

\todo{Überblick geben was nun passiert. Abbildung sollten wir wahrscheinlich hier schon vorwegnehmen.}
...
Starten wir mit $\neg\hQ$, dann haben wir über Satz~\ref{thm:q} auch die Negation von Aussage~\ref{thm:q}(8). Nachdem $\mathtt{rSAT}$ eine $\leqlp$-vollständige Levin-paddable NP-Relation ist, muss auch eine NPTM $N$ existiere, für die $L(N)=\mathtt{SAT}$, aber für alle Funktionen $h\in\FP$ gilt
...
\begin{theorem}[Äquivalente Formulierungen der Hypothese $\hQ$; \cites{fenner_inverting_2003}{messner_simulation_2001}]\label{thm:q}
    Folgende Aussagen sind äquivalent:
    \begin{enumerate}
...
\item $\hUP \Rightarrow \hTAUT$ nach \textcite{kobler_optimal_2003}. \todo{Relativierung?}
...
\item $\hNPcoNP\Rightarrow \hTAUT\lor\hSAT$ nach \textcite[Cor.~24]{beyersdorff_nondeterministic_2009}. \todo{Relativierung?}
...
\item $\mathsf \hSAT \Rightarrow \neg\hQ $ klar: wen $\hQ$, dann ist nach Satz~\ref{thm:q} jedes optimale Beweissystem auch p-optimal. Dann gilt mit Satz~\ref{} auch $\neg\hSAT$, denn wenn zu beliebiger Menge $L$ ein optimales Beweissystem $h$ existiert, dann ist $h$ auch p-optimal und $L$ hat ein p-optimales Beweissystem. \todo{Vielleicht als eigenständiges Korollar herausstellen.}
