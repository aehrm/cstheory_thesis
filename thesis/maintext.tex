...
\begin{proof}
    ...
    \begin{algorithm}[H]
        \uIf{$T$ kein Transduktor ist}{akzeptiere}
...
\begin{observation}\phantomsection\label{obs:partialoracles}
    \begin{enumerate}
...
\begin{itemize}
    ...
    \item $\tau^1_j$: Setze $t'=t_{s-1}\cup\{\tau^1_j\mapsto 0\}$. Existiert ein $t'$-valides Orakel $v\sqsupsetneq w_{s-1}$, dann setze $t_s\coloneqq t'$ und $w_s\coloneqq v$.
        ...
        Ansonsten setze $t_s\coloneqq t_{s-1}$ und setze $w_s\coloneqq w_{s-1}y$ für geeignetes $y\in\{0,1\}$ sodass $w_s$ auch $t_s$-valide ist. (Das ist möglich nach Behauptung~\ref{claim:oracle-up-extension}.)
    ...
    \item $\tau^2_{a,b}$: Setze $t'=t_{s-1}\cup\{\tau^2_{a,b}\mapsto 0\}$. Existiert ein $t'$-valides Orakel $v\sqsupsetneq w_{s-1}$, dann setze $t_s\coloneqq t'$ und $w_s\coloneqq v$. Entferne außerdem alle Tasks der Form $\tau^2_{a,b,r}$ von $T$.
        ...
        Ansonsten wähle ein hinreichend großes $m\not\in \img(t_s)$ sodass $w_s$ kein Wort der Länge $\min H_m$ definiert. Setze $t_s\coloneqq t_{s-1}\cup \{ \tau^2_{a,b}\mapsto m \}$; damit ist $w_{s-1}$ auch $t_s$-valide. Setze $w_s\coloneqq w_{s-1}y$ für geeignetes $y\in\{0,1\}$ sodass $w_s$ auch $t_s$-valide ist. (Das ist möglich nach Behauptung~\ref{claim:oracle-up-extension}.)
...
\begin{lemma}\label{claim:oracle-up-extension}
    ...
    \begin{enumerate}
        ...
        \item In allen anderen Fällen ist $w0$ und $w1$ $t$-valide.
    \end{enumerate}
\end{lemma}
\begin{proof}
    ...
    %Das widerspricht der $t$-Validität von $w$; im Fall (1) kann also V5 nicht verletzt sein.

    %Also gilt auch $z\in\Sigma^n$ und wir müssen in Fall (2) des Lemmas sein.
    %Hier gilt nun wahlweise $b=0$ und
    %\[ |w0\cap\Sigma^n|>1 \implies |w\cap\Sigma^n|>1, \]
    %haben also wieder einen Widerspruch zur $t$-Validität von $w$.
    %Oder $b=1$, dann gilt nach Voraussetzung von Fall (2b) auch $w\cap\Sigma^n=\emptyset$ und
    %\[ |w1\cap\Sigma^n|>1 \implies |w\cap\Sigma^n|>0 \implies w\cap\Sigma^n\neq \emptyset, \]
    %wieder Widerspruch.

    %Also kann nur noch V2 oder V7 verletzt sein. Hier lässt sich auf analoge Weise wie eben bei V5 sehen, dass das zu einem Widerspruch führt.
...
\end{lemma}
\begin{proof}[Skizze.]
    Widerspruchsbeweis. Erweitere $w_{s-1}$ so weit zu $u$, dass genau alle Wörter der Länge $<n=e(i)\in H_m$ definiert sind, wobei das $i$ hinreichend groß gewählt wird. Sei $u(X), X\subseteq \Sigma^n$ das Orakel was entsteht, wenn die Ebene $e(i)$ mit genau den Wörtern aus $X$ gefüllt wird, bzw. $u(X)=u\cup X \cup C$. Beob. dass $u(X), |X|\leq 1$ auch $t_{s-1}$-valide ist.

    Nach Annahme gilt
    \begin{itemize}[nosep]
        \item für $\alpha\in \Sigma^{n-1}0$ gilt $0^n\in A_m^{u(\{\alpha\})}$ und daher akzeptiert $M_a(F_r(0^n))$ definitiv relativ zu $u(\{\alpha\})$.
        \item für $\beta\in \Sigma^{n-1}1$ gilt $0^n\in B_m^{u(\{\alpha\})}$ und daher akzeptiert $M_b(F_r(0^n))$ definitiv relativ zu $u(\{\beta\})$.
    \end{itemize}
...
Wir haben $m = t_{\hat{s}}(\tau^2_{a,b}) = t_s(\tau^2_{a,b})$; fixiere auch $m$ für den Rest des Beweises.

Wir nehmen an, dass für alle $t_{s}$-gültigen $w\sqsupsetneq w_{s-1}$ weder (1) noch (2) zutrifft.
Daraus werden wir einen Widerspruch ableiten, indem wir ein geeignetes Orakel $u'\sqsupsetneq w_{\hat{s}-1}$ konstruieren, das bezüglich $t' \coloneqq  t_{\hat{s}-1}\cup \{\tau^2_{i,j}\mapsto 0\}$ valide ist. (Gemeint ist: relativ zu $u'$ wird $M_a$ und $M_b$ eine Eingabe definitiv akzeptieren.)
Dann folgt nach Definition, dass $u'$ eine mögliche $t'$-valide Erweiterung von $w_{\hat{s}-1}$ in Stufe $\hat{s}$ ist, daher hätte die Bearbeitung von $\tau^2_{a,b}$ eben $t_{\hat{s}}=t'$ gesetzt, was der Voraussetzung widerspricht.

Sei
\begin{equation*} \gamma(n) \coloneqq  \max(p_a(p_r(n))+p_r(n), p_b(p_r(n))+p_r(n)) \end{equation*}
...
Das bedeutet immer dann, wenn ein partielles Orakel $u'$ für alle Wörter der Länge $\leq \gamma(n)$ definiert ist, auch $M_i^{u'}(F_r^{u'}(x))$, $M_j^{u'}(F_r^{u'}(x))$ für alle Eingaben $x\in\Sigma^n$ definit sind.
...
Beachte, dass $\ell(Q)\leq\gamma(n)$ für $Q$ die Menge der Orakelqueries ist, die jeweils von der Berechnung $M_i(F_r(0^n))$ oder der Berechnung $M_j(F_r(0^n))$ gestellt werden.
Wir definieren nun $u\sqsupseteq w_{s-1}$ als ein $t_{s}$-gültiges partielles Orakel, das genau für alle Wörter bis zur Länge $<n$ definiert ist. Ein solches Orakel existiert nach Lemma~\ref{claim:oracle-up-extension}, indem man $w_{s-1}$ bitweise erweitert, so dass es $t_{s}$-gültig bleibt.

Für unseren Beweis betrachten wir nicht alle $t_s$-gültigen $w$, sondern vielmehr eine ausreichende Teilmenge davon.
...
Es ist leicht zu sehen, dass für $|X|\leq 1$ das Orakel $u(X)$ sogar $t_s$-valide ist, indem iterativ $u$ erweitert wird.

\begin{claim}
    Sei $|X|\leq 1$. Dann ist $u(X)$ ein $t_s$-valides Orakel.
\end{claim}
\begin{proof}
...
Wir zeigen induktiv, dass jedes $u_k$, $0\leq k\leq l$, $t_s$-valide ist.
Daraus folgt, dass $u_l=u(X)$ auch $t_s$-valide ist.
Der Basisfall ist klar, da $u_0 = u$ nach Konstruktion $t_s$-valide ist.

Für den Induktionsfall von $u_k$ zu $u_{k+1}$ sei $z=|u_k|$ und $y=u(X)[z]$. Es gilt also $u_ky=u_{k+1}\sqsubseteq u(X)$, das heißt, $y$ ist das letzte Bit von $u_{k+1}$.
Beachte dass $n\leq |y|\leq \gamma(n)$.
Das Orakel $u_k$ ist nach Induktionshypothese $t_s$-valide.
Wir zeigen nun aufgeteilt in zwei Fälle, dass $u_ky=u_{k+1}$ auch $t_s$-valide ist. Hierfür werden wir Lemma~\ref{claim:oracle-up-extension} in Bezug auf das $t_s$-valide $u_k$ verwenden.

Gilt $n<|y|\leq \gamma(n)$, dann wissen wir nach Definition dass $y=C[y]$. Da nach \eqref{eq:oracle1-expbound} insbesondere $n<|y|<2^n$ gilt, wissen wir auch dass $|y|\neq e(\cdot)$. Damit ist Lemma~\ref{claim:oracle-up-extension}(1) anwendbar, und es folgt dass $u_ky$ auch $t_s$-valide ist (gilt ja $y=1$ genau dann wenn $y\in C$).

Gilt ansonsten $|y|=n$, dann gilt nach Definition dass $y=X[y]$. Insbesondere gilt $|y|=n\in H_m$, wobei $m=t_{s-1}(\tau^2_{a,b})$ nach Wahl. %Damit ist Lemma~\ref{claim:oracle-up-extension}(2) anwendbar.
Gilt nun $u_k\cap\Sigma^n=\emptyset$, dann sind wir schon fertig da nur Fall (4) von Lemma~\ref{claim:oracle-up-extension} anwendbar ist, also $u_ky$ auch $t_s$-valide.

Gilt stattdessen $u_k\cap\Sigma^n\neq\empsyset$, dann muss auch $y=0$. Ansonsten wäre
\[ |u_{k}1\cap\Sigma^{e(i)}|>1 \implies |u(X)\cap\Sigma^{e(i)}|>1 \implies |X|>1, \]
was ein Widerspruch zur Wahl von $X$ ist.
Damit ist $y=0$ und Lemma~\ref{claim:oracle-up-extension}(3) anwendbar, und damit $u_k0=u_ky$ auch $t_s$-valide wie gewünscht.
\end{proof}

Wir haben angenommen, dass für $t_s$-valide Orakel nicht (1) oder (2) gilt. Angewendet auf $u(X), |X|\leq 1$ bedeutet das
\begin{itemize}[nosep]
...
Setze nun $u'=u(\{\alpha, \beta\})$. Wir zeigen nun zwei Dinge: erstens, dass $M_a(F_r(0^n))$ und $M_b(F_r(0^n))$ definitiv relativ zu $u(\{\alpha, \beta\})$ akzeptieren.
Zweitens anschließend, dass $u'$ auch $t'$-valide ist.
...
für ein geeignetes $l\in\mathbb N$.
Wir zeigen induktiv, dass jedes $u_k$, $0\leq k\leq l$, $t_s$-valide ist.
Der Basisfall ist klar, da $u_0 = u$ nach Konstruktion $t_s$-valide ist, und damit ist leicht zu sehen dass $u$ auch $t_{\hat{s}-1}$-valide ist.

Für den Induktionsfall von $u_k$ zu $u_{k+1}$ sei $z=|u_k|$ und $y=u(X)[z]$. 
Beachte wieder dass $n\leq |y|\leq \gamma(n)$.
Das Orakel $u_k$ ist nach Induktionshypothese $t_{\hat{s}-1}$-valide.

Gilt $n<|y|\leq \gamma(n)$, dann wissen wir nach Definition dass $y=C[y]$. Wie bei Beweis von Behauptung~\ref{} ist Lemma~\ref{claim:oracle-up-extension}(1) anwendbar, und es folgt dass $u_ky$ auch $t_{\hat{s}-1}$-valide ist.
...
Da also $u'\sqsupsetneq w_{\hat{s}-1}$ eben $t'$-valide ist, sind wir dann auch schon fertig und erreichen einen Widerspruch, wie anfangs argumentiert.
\end{proof}
...
\begin{proof}[Skizze.]
    ...
    Sei $t'=t_{s-1}\cup \{\tau^1_j\mapsto 0\}$ und sei $v=T\cap\Sigma^{\leq m}$ wobei $m\geq p_j(|x|), n+1$. 
    Beob. dass $M_j^v(z)$ definitiv ablehnt.

    Wir zeigen, dass $v$ auch $t'$-valide ist;  damit wäre $v$ eine geeignete Erweiterung in Stufe $s$ und wir hätten $t_s=t'$. 
    Das bedeutet nach V3, dass $M_j^{w_s}(z)$ definitiv ablehnt, damit auch $M_j^O(z)$ ablehnt, was der Voraussetzung widerspricht.

    Nun verifizieren wir diese $t'$-Validität:
