...
\begin{enumerate}[label=\arabic*.,nosep]
    ...
    \item Für jede Orakel-PTM $M$ existiert ein $i$ sodass $L(M^D_i)=L(M^D)$, 
        ...
        %Insbesondere kann jeder akzeptierender Rechenweg $\alpha$ einer Berechnung $N_i(x)$ in einen akzeptierenden Rechenweg $\alpha'$ der Berechnung $N(x)$ effizient übersetzt werden.
\end{enumerate}
...
Eine Stufe $s<\omega$ identifizieren wir hierbei mit einem partiellen Orakel $w_s\in\Sigma^*$, einer „Validitäts“-Funktion $t_s\in\mathcal T$, und einem Task (bzw. mehreren Tasks) welcher in dieser Stufe bearbeitet wurde.
...
\begin{description}
    ...
    \item[$\tau^2_{a,b}$:] Setze $t'\defeq t_{s-1}\cup\{\tau^2_{a,b}\mapsto 0\}$. Existiert ein $t'$-valides Orakel $v\sqsupsetneq w_{s-1}$, dann setze $t_s\defeq t'$ und $w_s\defeq v$. Entferne außerdem alle Tasks der Form $\tau^2_{a,b,r}$ von $T$.
    ...
    \item[$\tau^3_{a}$:] Setze $t'\defeq t_{s-1}\cup\{\tau^3_{a,b}\mapsto 0\}$. Existiert ein $t'$-valides Orakel $v\sqsupsetneq w_{s-1}$, dann setze $t_s\defeq t'$ und $w_s\defeq v$. Entferne außerdem alle Tasks der Form $\tau^3_{a,b,r}$ von $T$.
    ...
    \item[$\tau^3_{a,r}$:] Wir wissen dass $t_{s-1}(\tau^3_{a})=m>0$. Setze $t_s\defeq t_{s-1}$ und wähle ein $t_s$-valides Orakel $w_s\sqsupsetneq w_{s-1}$ sodass bezüglich einem $n\in\mathbb N$ eine der folgenden Aussagen gilt:
        \begin{itemize}[nosep,endpenalty=10000]
...
\begin{proof}
    ...
    \item Zu (2): Nach Voraussetzung muss es ein $s$ geben, für das $t_s(\tau^1_j)=0$. Insbesondere ist $w_s$ auch $t_s$-valide, und mit V3 existiert ein $z$ sodass $N_j^{w_s}(z)$ definitiv ablehnt.
...
%Sei nun angenommen, dass wir effizient $D$ entscheiden könnten, dann wäre auch $\P^{O}=\NP^{O}$ und wir könnten einen akzeptierenden Rechenweg von $N_j^O(x)$ einfach ausrechnen.
%Nun können wir zwar $D$ nicht effizient entscheiden, aber wir können -- mittels $N_j$ und Behauptung~\ref{} -- zunmindest iterativ einen relevanten Teil von $D$ bestimmen.
...
\begin{proof}
    ...
    Damit gilt: 
\begin{equation} \begin{split}&T\subseteq O, \text{$T$ stimmt mit Wörtern der Länge $\neq e(\cdot)$ und der Länge $\leq n$ mit $O$ überein} \\ &\implies L(N_j^T)=\Sigma^*.\end{split}\label{eq:oracle-1a} \end{equation}
...
Beob. dass damit $C\cup D\subseteq O$ und mit $O$ auf Wörtern der Länge $\neq e(\cdot)$ und Wörtern der Länge $\leq n$ übereinstimmt, also nach \eqref{eq:oracle-1a} auch $L(N_j^{C\cup D})=\Sigma^*$.
    ...
    Wie oben argumentiert gilt außerdem, dass $C\cup D\subseteq O$ und mit $O$ auf Wörtern der Länge $\neq e(\cdot)$ und Wörtern der Länge $\leq n$ übereinstimmt. Damit nach \eqref{eq:oracle-1a} auch $L(N_j^{C\cup D})=\Sigma^*$ und insbesondere existiert dann auch ein akzeptierender Rechenweg auf $N_j^{C\cup D}(x)$. Damit ist Z.~3 wohldefiniert.
    ...
    Damit stimmt $C\cup D$ mit $O$ auf $Q$ überein, und auch $N_j^O(x)$ akzeptiert mit Rechenweg $\alpha$.
