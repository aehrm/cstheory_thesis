...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        %\item (Generizität.) Für alle $\gamma\in\mathcal G$ \ldots{}
...
\begin{proof}
    ...
    %Starte mit $\gamma_{-1}=\gamma$. Für jedes $i\geq 0$, gegeben $\gamma_{i-1}$, bestimme zunächst (nach Definition 1(1)) das kleinste $n_{i-1}\in\omega$ sodass $(\exists A,B\in\gamma_{i-1})A(n)\neq B(n)$. 
    %Nach Definition 1(2) existiert dann eine $\mathcal G$-Bedingung $\sigma_{i-1,0}\subseteq \{ A\in \gamma_{i-1} \mid A(n)=0 \} \subseteq \gamma_{i-1}$.
    %Wähle nun (nach Lemma ??) eine $\mathcal G$-Bedingung $\gamma_i$ welche $\sigma_{i-1,0}$ erweitert und $\gamma_i \Vdash \phi_i\lor\neg\phi_i$.


    %%wähle zunächst ein $\sigma_0\in\mathcal G$ welches $\gamma_{i-1}$ erweitert. wähle ein $\gamma_i\in\mathcal G$ was $\gamma_{i-1}$ erweitert und sodass $\gamma_i \Vdash \phi_i\lor\neg\phi_i$. Dieses $\gamma_i$ existiert nach Lemma ??.
    ...
    %Außerdem gilt $\bigcap \mathbb G \neq \emptyset$. Wir konstruieren hierzu ein $G\in\bigcap \gamma_i$.

    %Folgende Beobachtung: Sei $i\in\omega$, dann stimmen alle $A\in \gamma_i$ auf den Stellen $0,1,\dots, n_i-1$ überein. Bezeichne dieses initiale Segment von $\gamma_i$ mit $a_i$.
    %Beobachte dass $a_{i}$ ein echter Präfix von $a_{i+1}$ ist, denn die $A\in \gamma_i$ stimmten bis Stelle $n_i-1$ überein, und die $A'\in\gamma_{i+1}\subseteq \gamma_i$ stimmen \emph{zusätzlich} auch noch mindestens auf Stelle $n_i$ überein.

    %Sei nun $G=\lim_{i\to\omega} a_i$. Angenommen $G\not\in y_i$ für ein $i<\omega$. Nach Konstruktion dann auch $G\not\in y_{i+1}, y_{i+2}, \dots$.
    %Dann gibt es ein kleinstes $n<\omega$ sodass für alle $A\in \gamma_i$ die Ungleichheit $A(n)\neq G(n)$ gilt.
